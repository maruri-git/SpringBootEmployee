...
public class CompanyControllerTest {
    ...
    void should_create_company_when_post_given_new_company() throws Exception {
        ...
        String companyStr = new ObjectMapper().writeValueAsString(companyService.addCompany(new Company(1, "spring", employeeList))); // remove add company
        //when then
        client.perform(MockMvcRequestBuilders.post("/companies")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(companyStr))
                .andExpect(MockMvcResultMatchers.status().isCreated())
                .andExpect(MockMvcResultMatchers.jsonPath("$.id", is(notNullValue()))) // id
                .andExpect(MockMvcResultMatchers.jsonPath("$.name", is("spring")));
    }
    ...
    void should_update_company_when_put_given_updated_company_and_id() throws Exception {
        //given
        Company company = companyService.addCompany(new Company(1, "spring", employeeList)); // remove add company
